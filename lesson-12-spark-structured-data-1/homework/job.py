...
def with_derived(events: DataFrame) -> DataFrame:
    ...
    return events.withColumn(
        "repo_owner", 
        F.substring_index(F.col("repo_name"), "/", 1)
    ).withColumn(
        "is_bot", 
...
def owner_totals(events: DataFrame) -> DataFrame:
    ...
    return events.groupBy("repo_owner").agg(
        ...
        F.count(F.when(F.col("is_bot") == True, 1)).alias("owner_bot_events")
    )
...
def top_repos_per_type(events: DataFrame, n: int) -> DataFrame:
    ...
    aggregated = events.groupBy("event_type", "repo_name").agg(
        F.count("*").alias("repo_event_count")
    )
    
    window_spec = Window.partitionBy("event_type").orderBy(
...
def enrich_top_repos(top_repos: DataFrame, owners: DataFrame) -> DataFrame:
    ...
    return (
        top_with_owner.join(owners, on="repo_owner", how="left")
        .withColumn(
            "owner_events", 
...
def build_summary(events: DataFrame, dimensions: list[str]) -> DataFrame:
    ...
    summary_df = summary_slice(events, dimensions[0])
    
    for dimension in dimensions[1:]:
        current_slice = summary_slice(events, dimension)
        summary_df = summary_df.unionByName(current_slice)
...
def write_outputs(outputs: dict[str, tuple[DataFrame, str | None]]) -> None:
    ...
    for name, config in outputs.items():
        ...
        if partition_col is None:
            ...
            df_repartitioned = df.repartition(partition_col)
            (
